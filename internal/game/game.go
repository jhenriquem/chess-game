...
func Run(id string) {
	...
	for _, p := range g.Players {
		...
		protocol.SendMessage(p.Client, msg, format.ToFormatGame(g))
	}
	...
	for {
		...
		case player := <-GetOne(id).Desconnect:
			...
			if player.Color == "black" {
				protocol.SendMessage(GetPlayer(g.ID, "white").Client, msg, format.ToFormatGame(g))

				GetPlayer(g.ID, "white").Client.WriteMessage(websocket.CloseMessage, []byte{})
				GetPlayer(g.ID, "white").Client.Close()
			} else {
				protocol.SendMessage(GetPlayer(g.ID, "black").Client, msg, format.ToFormatGame(g))
